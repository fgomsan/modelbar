...
Quit ModelBar first if it is already open, or you will get two menu-bar icons. The login item does not inherit your shell; it only sets `HOME` and a small `PATH` (including `~/.lmstudio/bin` and `~/.omlx/bin`). Extra folders are opt-in: export `MODELBAR_DS4_DIR` and/or `MODELBAR_GGUF_DIRS` before running the script, or add them later to `~/Library/LaunchAgents/com.fgs.model-bar.plist` (colon-separated, tildes and spaces allowed) and reload the agent. oMLX on a non-default port is picked up from `~/.omlx/settings.json` or `omlx serve --port`; override with `MODELBAR_OMLX_PORT` if you need to.
...
- oMLX: HTTP `GET` only (never `POST /load` or `/unload`, never `omlx start`). Port from `MODELBAR_OMLX_PORT` / `OMLX_PORT`, then `omlx serve --port` in `ps`, then `~/.omlx/settings.json` (`server.port`). Default is `:8000`, then `:8083`, only when oMLX is present. If `ds4-server` is up, `:8000` is left to DS4 unless oMLX’s own settings/`--port` say otherwise. The port is confirmed with `GET /health` (`engine_pool`) so DS4 on `:8000` is not stolen. Prefers `GET /v1/models/status` (with `Authorization: Bearer` from `MODELBAR_OMLX_API_KEY` / `OMLX_API_KEY` / `auth.api_key` in settings if set), then `/admin/api/models`, then `/v1/models`. Rows with `loaded: true` go under In RAM **and stay under On disk**. Helper / MarkItDown / hidden draft rows are skipped; DeepSeek is not treated as a draft. HTTP 401 is “set a token or turn off the API key”, same idea as LMS. If HTTP is down, list `~/.omlx/models` plus `model.model_dirs` from settings (directories with `config.json` and safetensors, plus GGUF in those folders). Process up but the port not ready is a “starting” note, not a loaded row. Click warns; ModelBar never starts, stops, or POSTs to oMLX.
...
- oMLX In RAM: `actual_size` when oMLX reports it, otherwise `estimated_size` or `engine_pool.current_model_memory`. If `/v1/models` has no load flag, RSS of the `omlx` process only when it is already over 2 GiB (shown as `oMLX`, not as a guessed library row).
